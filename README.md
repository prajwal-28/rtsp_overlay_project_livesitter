...
A video stream is served over RTSP (local RTSP server).

FFmpeg converts the RTSP stream into HLS format (.m3u8 + .ts).

A custom HTTP server serves the HLS files with correct CORS and MIME headers.

The React frontend loads the HLS stream using hls.js.

Users can add text or image overlays on top of the video.

Overlay positions, sizes, and content are stored via Flask APIs.

On page reload, overlays are restored from the database.
...
Add image overlay using image URL
